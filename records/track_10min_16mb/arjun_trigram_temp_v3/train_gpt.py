...
class TrigramHashEmbedding(nn.Module):
    ...
    Position 0 and 1 use fallback bucket (no full trigram available).
    Cheap: 2048 buckets x 64 dim x 6/8 bytes int6 x 0.5 lzma ~ 49KB artifact cost."""
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        if t.shape[-1] > 2:
            h = (48271 * t[..., 2:] ^ 36313 * t[..., 1:-1] ^ 27191 * t[..., :-2]) % mod
            out[..., 2:] = h
        return out.long()
...
def eval_val_sliding_ttt(
    ...
    # TTT temperature calibration: fit scalar T on already-scored chunk, apply to next
    _ttt_temp = torch.tensor(1.0, device=device, dtype=torch.float32)
    _ttt_nll_sum = torch.zeros((), device=device, dtype=torch.float64)
    _ttt_nll_count = torch.zeros((), device=device, dtype=torch.float64)

    def _fit_temp(nll_sum: torch.Tensor, count: torch.Tensor) -> torch.Tensor:
        if count.item() < 1:
            return torch.tensor(1.0, device=device, dtype=torch.float32)
        avg_nll = nll_sum.item() / count.item()
        best_t, best_loss = 1.0, avg_nll
        for t in [0.80, 0.85, 0.90, 0.95, 1.00]:
            approx = avg_nll / t  # first-order approx valid near T=1
            if approx < best_loss:
                best_loss, best_t = approx, t
        return torch.tensor(best_t, device=device, dtype=torch.float32)
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                if _ttt_temp.item() != 1.0:
                    logits = logits / _ttt_temp.to(dtype=logits.dtype)
                nll = F.cross_entropy(
                    logits.reshape(-1, logits.size(-1)).float(),
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    loss_sum += scored_nll.sum()
                    token_count += float(wlen - s)
                    _ttt_nll_sum += scored_nll.sum()
                    _ttt_nll_count += float(wlen - s)
                    tgt, prev = y_batch[i, s:wlen], x_batch[i, s:wlen]
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()

        # Fit temperature on this chunk for next chunk
        _ttt_temp = _fit_temp(_ttt_nll_sum, _ttt_nll_count)
        if rank == 0 and ci % 20 == 0:
            log0(f"  ttt_temp:chunk_{ci} T={_ttt_temp.item():.3f}")
